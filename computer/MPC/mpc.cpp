...
void MPC::setMPCProblem(int model_type)
{
    for(int i=0;i<=N;i++)
    {    
        setStage(initial_guess_[i].xk,initial_guess_[i].uk,i,model_type);
    }
}

void MPC::setStage(const State &xk, const Input &uk, const int time_step, int model_type)
{
    ...
    // ************************************************************************************
    // Use model 
    file << "step: " << time_step << std::endl;
    file << "vx " << xk_nz.vx << " vy: " << xk_nz.vy << " r: " << xk_nz.r;
    file << " D: " << xk_nz.D << " delta: " << xk_nz.delta << std::endl;
    file << "dD: " << uk.dD << " dDelta: " << uk.dDelta << std::endl;
...
{      
    // all the guess shift left 
    // the 0 and N-1 are set 
    // the N are set by updating the N-1
    // unwrap guess  
    ...
    initial_guess_[N].xk = integrator_.EF(initial_guess_[N-1].xk,initial_guess_[N-1].uk,Ts_);
...
// fuse the last solution and current solution 
...
// *******************************************************************************************************
// Get initial guess : update or generate new when the solver cannot find solutions 
// set MPC problem to get stage 
// input stage and x0 to get the optimal solution 
// combine the optimal solution with the initial guess
// check if the solver works fine 
// return guess and time span
MPCReturn MPC::runMPC(State &x0, int n, int model_type)
{
